...
// Requirements: ASCII character 'a', 38400 bps, 8O1 format (8 data bits, Odd
// Parity, 1 Stop bit)
...
#define SYSTEM_CLOCK                                                           \
  16000000U // System clock in Hz (I'm assuming 16 MHz without PLL)

volatile static int wait_counter_1b =
    0; // 'volatile' to prevent compiler optimization if used as a simple delay
       // loop
 ...
 *                 Configured for 801 format (8 data bits, Odd Parity, 1 Stop
 * bit). FIFOs are disabled according to the LCRH setting (0x62).
 */
void config_uart_1b(uint32_t baudrate) {
  ...
  }; // Wait until bit 6 of UART6 (PRUART) is set. Bit-shift //
     // while((SYSCTL_PRUART_R & (1U << 6)) == 0) {};
  ...
  // Configure serial parameters: 8 data bits, Odd Parity, 1 Stop bit (8O1),
  // FIFO disabled UART6_LCRH_R: WLEN (Bits 6:5) = 0b11 (8 data bits) FEN  (Bit
  // 4)    = 0 (FIFO disabled) STP2 (Bit 3)    = 0 (1 Stop bit) EPS  (Bit 2) = 0
  // (Odd Parity Select) PEN  (Bit 1)    = 1 (Parity Enable) Value: (0x3 << 5) |
  // (1 << 1) = 0x60 | 0x02 = 0x62
...
void run_aufgabe_1b(char zeichen, uint32_t baudrate) {
  ...
  while (1) {
    ...
    while ((UART6_FR_R & 0x20) != 0)
      ;                   // Explizit und Sicher	// till transmit FIFO not full
 ...
 */
void init_aufgabe_1b(void) { config_port_1b(); }

/**
 * @brief Sends a single character via UART6.
 *
 * Configures UART6 on the first execution or if the baud rate changes.
 * Waits until the transmit FIFO has space, sends the character, and then waits
 * for a defined time.
 *
 * @param zeichen The character to send.
...
void execute_aufgabe_1b(char zeichen, uint32_t baudrate) {
  // Statische Variable, um sicherzustellen, dass die UART nur einmal
  // konfiguriert wird
