...
void config_uart_1b(void){
    ...
    UART6_FBRD_R = (16000000/(16*BPS))*64; 	// set DIVFRAC of BRD remaining fraction divider
    UART6_LCRH_R = 0x00000062; 			// serial format 8O1
...
void run_aufgabe_1b(void){
    config_port_1b(); 				// configuration of Port P
    config_uart_1b(); 				// configuration of UART6
    while(1){
        while((UART6_FR_R & 0x20) !=0); 	// till transmit FIFO not full
        printf("Aufgabe 1b\n");
        UART6_DR_R = 'a';			// send the character 'a'
        idle_1b(); 				// idle time
    }
}
