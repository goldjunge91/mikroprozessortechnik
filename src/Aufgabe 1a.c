...
// Requirements: ASCII character 'X', 9600 bps, 7E1 format (7 data bits, Even
// Parity, 1 Stop bit)
 ...
 *                 Configured for 7E1 format (7 data bits, Even Parity, 1 Stop
 * bit). FIFOs are disabled according to the current LCRH setting (0x46).
 */
void config_uart_1a(uint32_t baudrate) {
  ...
  // Configure serial parameters: 7 data bits, Even Parity, 1 Stop bit (7E1),
  // FIFO disabled UART6_LCRH_R: WLEN (Bits 6:5) = 0b10 (7 data bits) FEN  (Bit
  // 4)    = 0 (FIFO disabled) STP2 (Bit 3)    = 0 (1 Stop bit) EPS  (Bit 2) = 1
  // (Even Parity) PEN  (Bit 1)    = 1 (Parity Enable) BRK  (Bit 0)    = 0 (No
  // Break) Value: (0x2 << 5) | (1 << 2) | (1 << 1) = 0x40 | 0x04 | 0x02 = 0x46
...
void execute_aufgabe_1a(char zeichen, uint32_t baudrate) {
  ...
  while ((UART6_FR_R & 0x20) != 0)
    ; // Wait until the transmit FIFO is not full (TXFF flag, Bit 5 in UARTFR
      // register)
